...
function useLiveRooms(rooms, thresholds) {
  const [roomData, setRoomData] = useState(() =>
    rooms.map((r) => ({
      ...
      params: Object.fromEntries(
        DEFAULT_PARAM_DEFS.map((p) => [
          p.key,
          {
            value: thresholds[p.key]?.base ?? p.base,
            history: seedHistory(thresholds[p.key]?.base ?? p.base, p.jitter),
          },
        ])
      ),
    }))
      ...
      setRoomData((prev) =>
        prev.map((room) => {
          ...
          DEFAULT_PARAM_DEFS.forEach((p) => {
            ...
            const base = thresholds[p.key]?.base ?? p.base;
            ...
            if (p.key === "temp" && room.wired) {
              params[p.key].value = Number(Math.max(low, Math.min(high, params[p.key].value)).toFixed(2));
            }
          });
...
function RoomRow({ room, active, onSelect }) {
  const worst = useMemo(() => {
    let w = "ok";
    DEFAULT_PARAM_DEFS.forEach((p) => {
...
export default function PanelRoomMonitor({ rooms = [], thresholds = {} }) {
  const roomsData = useLiveRooms(rooms, thresholds);
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "220px 1fr 260px", gap: 14, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 15, color: COLORS.textHi, fontFamily: "'Space Grotesk', sans-serif" }}>
                {selected ? selected.name : "No room"}
              </div>
              <div style={{ fontSize: 11.5, color: COLORS.textLow }}>{selected ? selected.zone : ""}</div>
            </div>
            <div style={{ fontSize: 11, color: COLORS.textLow, fontFamily: "'IBM Plex Mono', monospace" }}>
              {selected && selected.wired ? "wired · RS485/Modbus" : "wireless · ESP32/MQTT via VPN"}
            </div>
          </div>
          ...
          <HeartbeatStrip rooms={roomsData} />
        </div>
